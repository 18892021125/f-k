...
std::string generate_texture_views(
	...
	std::vector<TextureView> * texture_views)
{
	ProgressCounter view_counter("\tLoading", imagesData.size() / 2);
	for (auto i = 0; i < imagesData.size(); ++i)
	{
		mve::CameraInfo cam_info;
		memcpy(cam_info.rot, camerasExtrinsic[i].data() + 3, sizeof(cam_info.rot));
		memcpy(cam_info.trans, camerasExtrinsic[i].data(), sizeof(cam_info.trans));
		cam_info.flen = camerasIntrinsic[i][0];
		cam_info.dist[0] = camerasIntrinsic[i][1];
		cam_info.dist[1] = camerasIntrinsic[i][2];
		cam_info.paspect = camerasIntrinsic[i][3];
		cam_info.ppoint[0] = camerasIntrinsic[i][4];
		cam_info.ppoint[1] = camerasIntrinsic[i][5];

		auto image = mve::ByteImage::create();
		image->allocate(width, height, 3);
		auto data = image->get_data_pointer();
		//image->get_data().data()¿½²»¹ýÈ¥
		memcpy(data, imagesData[i].data(), imagesData[i].size());
		texture_views->push_back(TextureView(i, cam_info, image));
